...
| **FR-004** | 전·좌·우 **삼면이 동시에 막혀 있으면 후진**한다. 후진 후 측면 중 한쪽이 열리면 그쪽으로 회전한 뒤 전진 청소를 재개한다. 모든 후진/회전 시퀀스는 `CleaningCoordinator`가 **단일 오케스트레이션**으로 수행한다. | UC-004 | High |
...
| **NFR-TEST-001** | **단위·통합 GTest** 라인 커버리지 ≥ 80 %, 분기 커버리지 ≥ 70 %를 권장 게이트로 둔다(팀 합의 시 머지 차단). 시스템 테스트는 GTest로 작성하지 않는다. | `gcovr`/`llvm-cov` 산출 후 CI 아티팩트. | `unit_tests`, `integration_tests` job. |
